...
try {
 ...
 await db.connect();connected=true;await db.query(`CREATE SCHEMA "${schema}"`);created=true;await db.query(`COMMENT ON SCHEMA "${schema}" IS '${marker}'`);
 ...
 await run('go',['test','-p=1','-race','-tags=integration,shortlinks','-count=1','-run','TestShortLinks|TestShortLinkCollisions','./internal/httpapi','./internal/shortlink'],root);
 ...
 report.cleanup='complete';if(connected)await db.end();
 writeFileSync(resolve(artifacts,'verification.json'),JSON.stringify(report,null,2)+'\n');
 console.log(JSON.stringify({schema,status:report.status,cleanup:report.cleanup,browser:report.browser}));
}
