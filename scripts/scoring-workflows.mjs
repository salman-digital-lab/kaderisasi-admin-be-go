...
try {
  ...
  const panitia=(await fixture.db.query("INSERT INTO admin_users(email,normalized_email,password,display_name,is_active,role_code,created_at,updated_at) VALUES('panitia@example.test','panitia@example.test',$1,'Panitia scoring fixture',true,'activity_manager',now(),now()) RETURNING id",[passwordHash])).rows[0];
  panitiaToken=legacyRequire('jsonwebtoken').sign({userId:panitia.id,email:'panitia@example.test'},fixtureKey,{expiresIn:'1h',audience:'kaderisasi-admin'});
  ...
  await call('admin','Panitia cannot open registration','PUT',`/activities/${activity.id}`,{is_registration_open:true},{token:panitiaToken,status:403});
  await call('admin','Panitia cannot publish activity','PUT',`/activities/${activity.id}`,{is_published:true},{token:panitiaToken,status:403});
  ...
  await call('admin','save partial draft','PUT',`${base}/registrations/${registration.id}`,draft(0,{honesty:100}),{token:panitiaToken});
  ...
  await call('admin','publish owner','POST',`${base}/publish`,batch([registration.id,2]),{token:panitiaToken});
  ...
  await call('admin','withdraw result','POST',`${base}/withdraw`,batch([registration.id,6]),{token:panitiaToken});
  ...
  const preview=await call('admin','Excel preview','POST',`${base}/import/preview`,upload(buffer),{token:panitiaToken});assert.equal(preview.errors.length,0);assert.equal(preview.changes.length,1);
  await call('admin','Excel commit','POST',`${base}/import/commit`,upload(buffer,preview.hash),{token:panitiaToken});
