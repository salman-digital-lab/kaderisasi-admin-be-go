...
export async function detailShortLinksBrowser(page, db, schema, artifacts, shortBase='http://localhost:4000') {
 const publicBase=process.env.SHORT_LINK_TEST_PUBLIC_BASE??'http://127.0.0.1:3000';
 ...
 for(const [kind,path,destination] of [['activity','/activity/91001',`${publicBase}/activity/kegiatan-qr`],['club','/club/91002',`${publicBase}/clubs/91002`]]) {
  ...
  const short=await card.getByLabel('Tautan pendek halaman').inputValue();assert.equal(new URL(short).origin,shortBase);assert.match(new URL(short).pathname,/^\/[A-Za-z0-9]{6}$/);
