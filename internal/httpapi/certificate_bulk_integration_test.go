...
func TestCertificateBulkPausesOnConcurrentTemplateChange(t *testing.T) {
	...
	expected := certificate.Expectation{ActivityID: fixture.activity.ID("id"), TemplateID: fixture.template.ID("id"), TemplateVersion: 2}
	var once sync.Once
	var mutationErr error
	hook := issuanceLogHook{Handler: slog.NewTextHandler(io.Discard, nil), after: func() error {
		once.Do(func() {
			_, mutationErr = f.pool.Exec(ctx, "UPDATE certificate_templates SET version=version+1 WHERE id=$1", expected.TemplateID)
		})
		return mutationErr
	}}
	...
	frozen, err := service.ByID(ctx, result.Created[0].Certificate.ID)
	if err != nil || frozen.Template.Version != 2 {
		t.Fatal("prior snapshot changed", err)
	}
	f.call("PUT", fmt.Sprintf("/v2/activities/%d", expected.ActivityID), map[string]interface{}{"certificate_template_id": nil}, f.token, 200)
...
func TestCertificateRecipientPaginationWithDuplicateProfiles(t *testing.T) {
	...
	page, err := service.Recipients(ctx, fixture.activity.ID("id"), certificate.RecipientOptions{Page: 2, PerPage: 50})
	if err != nil {
		t.Fatal(err)
	}
	if page.Meta.Total != 101 || len(page.Data) != 50 || page.Counts["eligible_not_issued"] != 101 {
		t.Fatal("recipient multiplication", page.Meta, page.Counts, len(page.Data))
	}
	filtered, err := service.Recipients(ctx, fixture.activity.ID("id"), certificate.RecipientOptions{Page: 1, PerPage: 50, Search: "Peserta 101"})
	if err != nil || len(filtered.Data) != 1 || filtered.Counts["eligible_not_issued"] != 101 {
		t.Fatal("search affected aggregate counts", err)
	}
	all, err := service.Prepare(ctx, fixture.activity.ID("id"), nil)
	if err != nil || !reflect.DeepEqual(all.RegistrationIDs, fixture.ids) {
		t.Fatal("whole-activity preparation", err)
	}
	selected, err := service.Prepare(ctx, fixture.activity.ID("id"), []int32{fixture.ids[0], fixture.ids[100]})
...
func TestCertificateThousandRecipientBatches(t *testing.T) {
	...
	review, err := (certificate.Issuance{Pool: empty.f.pool, Location: time.Local}).Prepare(ctx, empty.activity.ID("id"), nil)
	...
	if _, err = service.Revoke(ctx, first.Certificate.ID, "Synthetic fixture revocation", f.adminID, "fixture"); err != nil {
		t.Fatal(err)
	}
	if _, err = f.pool.Exec(ctx, "UPDATE activity_registrations SET status='BELUM LULUS' WHERE id=$1", fixture.ids[1]); err != nil {
		t.Fatal(err)
	...
	count, err := (database.JSONQueries{DB: f.pool}).Count(ctx, "SELECT id FROM issued_certificates WHERE activity_id=$1", expected.ActivityID)
	if err != nil || count != 1000 {
		t.Fatal("exactly one certificate per recipient", count, err)
	}
}

func wideIDs(ids []int32) []int64 {
