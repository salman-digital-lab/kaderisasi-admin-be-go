...
SELECT count(*) FROM issued_certificates c WHERE (sqlc.narg('activity_id')::text IS NULL OR c.activity_id=CAST(CAST(sqlc.narg('activity_id') AS text) AS integer)) AND (NOT @filter_selected::boolean OR c.registration_id=ANY(CAST(CAST(@registration_ids AS text[]) AS integer[])) AND c.id=(SELECT max(latest.id) FROM issued_certificates latest WHERE latest.registration_id=c.registration_id));
