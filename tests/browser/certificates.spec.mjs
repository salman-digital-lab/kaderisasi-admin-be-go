...
test('upload and publish template, issue, download, and revoke certificate',async({page,fixture},testInfo)=>{
  ...
  const signerName=(await fixture.db.query('SELECT display_name FROM admin_users WHERE id=1')).rows[0].display_name;
  await page.getByLabel('Penandatangan',{exact:true}).fill(signerName);
  await page.getByLabel('Penandatangan',{exact:true}).press('ArrowDown');
  await page.getByLabel('Penandatangan',{exact:true}).press('Enter');
  await page.getByLabel('Jabatan pada sertifikat',{exact:true}).fill('Ketua kegiatan');
  await page.getByRole('button',{name:'Kirim 1 permintaan persetujuan',exact:true}).click();
  await expect.poll(async()=>Number((await fixture.db.query('SELECT count(*) FROM certificate_approvals')).rows[0].count)).toBe(1);
  expect(Number((await fixture.db.query('SELECT count(*) FROM issued_certificates')).rows[0].count)).toBe(0);
  await page.getByRole('button',{name:'Tinjau',exact:true}).click();
  dialog=page.getByRole('dialog');
  await expect(dialog.getByRole('button',{name:'Setujui & terbitkan 1 sertifikat',exact:true})).toBeDisabled();
  await dialog.getByRole('checkbox').check();
  await dialog.getByRole('button',{name:'Setujui & terbitkan 1 sertifikat',exact:true}).click();
  await expect(dialog).not.toBeVisible();
