...
test('upload and publish template, issue, download, and revoke certificate',async({page,fixture},testInfo)=>{
  ...
  dialog=page.getByRole('dialog');
  await expect(dialog.getByText('Oktofa Yudha Sudrajad, S.T., M.S.M., Ph.D.',{exact:true})).toBeVisible();
  await dialog.getByRole('button',{name:'Terbitkan dan tanda tangani',exact:true}).click();
  await expect(dialog).not.toBeVisible();
  ...
  await dialog.getByRole('button',{name:'Batalkan penerbitan',exact:true}).click();
  await expect(dialog).not.toBeVisible();
